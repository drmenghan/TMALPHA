...
class Message(object):
    ...
    # Type = "X" #"X" for default, "E" for Email, "M" for Message, "U" for Unclassified
        self.Subject = "" #Kindes of Other, ERROR, NULL
        self.Content = ""
        self.SubjectFlag = False
        self.ReplyTimes = 0
        self.Reply = Reply()
        self.NullError = [False]*7 # For NullError Record
        self.CaseID = ""
...
def get_similarity(a, b):
    """
    Based on library to calculate the similarity between two terms
    :param a:
    :param b:
    :return:
    """
...
def get_converted_dic(MessageList):
    """
    Convert one list to dictionary to index the object
    :param MessageList:
    :return:
    """
    MessageListDic = dict()
...
def get_FCRlist(rawList, logfile):
    """
    Get FCR List
    The message with more than one replies will be considered as FCR message
    :param rawList:
    :param logfile:
    :return:
    """
...
def get_count_result(QsList, MessageList, logfile):
    """
    Count the number of messages in each subjects
    :param QsList:
    :param MessageList:
    :param logfile:
    :return:
...
def output_count_result(QsList):
    """
    Out put the count result in each subject
    :param QsList:
    :return:
    """
    for Qs in QsList:
...
def output_file(ObjectContent,filename,logfile):
    """
    Out put one object to a file with code utf8
    :param ObjectContent:
    :param filename:
    :param logfile:
    :return:
...
def input_file(filename,logfile):
    """
    Input a file, read it and return the content with standard text code
    :param filename:
    :param logfile:
    :return:
    """
...
def get_content_fraction(text):
    """
    Calculate the stop word and do the clearance.
    :param text:
...
def main():
    ...
    *******************************************************************************
    ...
    while ans:
        print("""
        This software is design for cleansing and analyzing (.txt) file for CSE of Discover Financial Services.
                                                Copyright 2015 Discover Bank, Member FDIC. All rights reserved.
        [1]Clean the data in specific directory.
        [2]Load categories information from Excel file.
        [3]Analysis data in specific directory.
        [4]Output the data to file in binary format.
        [5]Automatically initial the program and do data analysis.
        [6]Help


        """)
        ans = input("What would you like to do ? (Please input the number of option you want to go:)")
        if ans == '1':
            inputDic = input("Please input the directory stored all the raw data to clean:(0 for default)")
            print("The default raw data dictionary is", Dic1)
            print("Please input 0 for default setting.")
            if(inputDic == 0):
                Dic = Dic1
                FileList = get_filelist(Dic,logfile)
                fileCheck(FileList, Dic, logfile)
            else:
                print("Please input the correct file directory!")
        elif ans == '2':
            inputXLSFile = ("Please input the Excel file name (include .xlsx) to load the category information:(0 for default)")
            if(inputXLSFile == XLSFile or inputXLSFile == 0):
                print("The default XLS File is", XLSFile)
                print("Please input 0 for default setting.")
                CategoryInfo = get_category(XLSFile,logfile)
            else:
                print("Please input the correct file directory!")
        elif ans == 3:
            inputFurtherAnalysis = input("Please input the directory stored all the data you want to analysis:(0 for default)")
            print("The default raw data dictionary is", Dic1)
            print("Please input 0 for default setting.")
            if(inputFurtherAnalysis == 0):
                Dic = Dic1
                subjectFinder(FileList,Dic,logfile)
            else:
                print("Please input the correct file directory!")
        elif ans == 4:
            OtherObjectFile = "OtherObjectFile.pkl"
            OtherList = load_object(OtherObjectFile,logfile)
            output_to_raw_data(OtherList,"otherlistRawData.txt", logfile)
        elif ans == 5:
            print("\n")
        elif ans == 6:
            print("Please regarding to the user guide together with this project.")

        else:
            print("Not Valid Choice Try again:")
            again = input("Would you like to continue? (Y for yes)")
            if(again == "Y"):
                continue
            else:
                break
    ...
    *******************************************************************************
    ...
    #init dictionary store the raw data
    Dic = Dic1
    #index files in the specific folder
    FileList = get_filelist(Dic,logfile)
    #Clean the data, for each unanalyzable data, put them to different location.
    fileCheck(FileList, Dic, logfile)

    """
    -------------------------------------------------------------------------------
    *******************************************************************************
                            Second Stage: Analysis Data
    *******************************************************************************
    -------------------------------------------------------------------------------
    """
    ...
    -------------------------------------------------------------------------------
    *******************************************************************************
            Subject Analysis for both Messages with reply and without reply.
    *******************************************************************************
    -------------------------------------------------------------------------------
    """
    #For messages with reply
    ...
    *******************************************************************************
    ...
    -------------------------------------------------------------------------------
    *******************************************************************************
                    Analyze Data with and without reply by Dictionary
    *******************************************************************************
    -------------------------------------------------------------------------------
    """
    ...
    *******************************************************************************
    ...
    -------------------------------------------------------------------------------
    *******************************************************************************
                        Set up the categories information
    *******************************************************************************
    -------------------------------------------------------------------------------
    """
    ...
    *******************************************************************************
    ...
    #Build the questionListDictionary
    #Categories-Subjects
